    ...
    moduleSvc.factory('svc', ['$http', function($http) {
        return {
            ...
            update: function(model) {
                var def = $.Deferred();
                var promise = def.promise();
                $http.put('/requirement', model).success(function(json) {
            ...
            create: function(model) {
                var def = $.Deferred();
                var promise = def.promise();
                $http.post('/requirement', model).success(function(json) {
                    if (!json.status || json.status == 'fail') return def.reject(json ? json.msg : '未知的错误');
                    def.resolve(json.result);
                });
            ...
            retrieve:function() {
                ...
                $http.get('/requirement', {params : pager.condition}).success(function(json) {
                    if (!json) return def.reject('未知的错误');
                    if (!json.status || json.status == 'fail')  return def.reject(json.msg);
                    def.resolve(json);
                });
